...
export async function PUT(request, { params }) {
    try {
        ...
        const [rows] = await pool.query('SELECT id FROM ingresos WHERE id = ? AND usuario_id = ?', 
            [id, session.user.id]
        );
        ...
        await pool.query('UPDATE ingresos SET categoria_id = ?, cantidad = ?, descripcion = ?, fecha = ? WHERE id = ?',
            [categoria_id, cantidad, descripcion, fecha, id]
        );        

        return NextResponse.json(
            { mensaje: 'Ingreso modificado' },
            { status: 200 }
        );
