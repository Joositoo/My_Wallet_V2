...
export async function GET() {
    try {        
        ...
        const [AllExpenses] = await pool.query('SELECT * FROM gastos WHERE usuario_id = ?',
            [session.user.id]
        );
        const [currentMonth] = await pool.query('SELECT SUM(cantidad) FROM gastos WHERE usuario_id = ? AND MONTH(fecha) = MONTH(CURRENT_DATE()) AND YEAR(fecha) = YEAR(CURRENT_DATE())',
            [session.user.id]
        );
        const [currentYear] = await pool.query('SELECT SUM(cantidad) FROM gastos WHERE usuario_id = ? AND YEAR(fecha) = YEAR(CURRENT_DATE())',
            [session.user.id]
        )
...
export async function POST(request) {
    try {        
        ...
        const [result] = await pool.query('INSERT INTO gastos (usuario_id, categoria_id, cantidad, descripcion, fecha) VALUES (?, ?, ?, ?, ?)',
            [session.user.id, categoria_id, cantidad, descripcion.trim(), fecha]
        );

        return NextResponse.json(
            { mensaje: "Gasto creado", id: result.insertId },
            { status: 201 });
    }
    catch (error) {
        console.log(error);
